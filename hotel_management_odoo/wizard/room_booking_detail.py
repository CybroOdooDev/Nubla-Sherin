...
class RoomBookingWizard(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        for i in data["booking"]:
            ...
            sheet.write(row, column + 3, i["checkin_date"].strftime('%Y-%m-%d %H:%M:%S') if i["checkin_date"] else "", body)
            sheet.write(row, column + 4, i["checkout_date"].strftime('%Y-%m-%d %H:%M:%S') if i["checkout_date"] else "", body)
