...
class NhsTrustMergeWizard(models.TransientModel):
    ...
    def _check_reason(self):
        """Require a justification of at least 5 non-whitespace characters for the merger."""
        for wiz in self:
    ...
    def _check_merge_regions(self):
        """Forbid merging trusts that belong to different NHS Regions."""
        for wiz in self:
    ...
    def action_confirm_merge(self):
        """Log the merger, optionally transfer sites/board members from source to target, set the source trust to 'merging', and post a chatter summary on both trusts."""
        self.ensure_one()
...
class NhsTrustDissolveWizard(models.TransientModel):
    ...
    @api.constrains('reason')
    def _check_reason(self):
        """Require a justification of at least 5 non-whitespace characters for the dissolution."""
        for wiz in self:
            if not wiz.reason or len(wiz.reason.strip()) < 5:
    ...
    def action_confirm_dissolve(self):
        """Log the dissolution, optionally archive associated sites, clear board member links, set the trust to 'dissolved', and post a chatter summary."""
        self.ensure_one()
        trust = self.trust_id
