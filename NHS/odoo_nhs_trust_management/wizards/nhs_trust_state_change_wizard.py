...
class NhsTrustStateChangeWizard(models.TransientModel):
    ...
    def _check_reason(self):
        """Require a justification of at least 5 non-whitespace characters for the state change."""
        for wiz in self:
    ...
    def _check_state_transition(self):
        """Enforce the allowed workflow state graph, forbidding NHS Scotland trusts from entering Special Measures."""
        allowed_transitions = {
            'draft': ['under_review'],
            'under_review': ['active'],
    ...
    def action_confirm(self):
        """Log the state transition as an immutable audit record, then apply it to the trust."""
        self.ensure_one()
