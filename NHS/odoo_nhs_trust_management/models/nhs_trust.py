...
class NhsTrust(models.Model):
    ...
    def _check_ods_code(self):
        """Validate that ods_code is alphanumeric and matches the length/prefix rules for the Trust's health_system."""
        for trust in self:
    ...
    def _check_geographic_fields(self):
        """Validate that region/ICB/ICS/Health Board fields are consistent with each other and with health_system.

        Skipped entirely when nhs_ods_sync is set in context (used by automated ODS data sync).
        """
    ...
    def _check_state_health_system(self):
        """Prevent NHS Scotland Trusts from being placed in the England-only 'special_measures' state."""
        for trust in self:
    ...
    def _onchange_health_system(self):
        """Reset all geography fields and re-scope their domains, and Scotland-clear England-only governance fields."""
    ...
    def _onchange_country_id(self):
        """Clear state_id when it no longer belongs to the selected country."""
        if self.state_id and self.state_id.country_id != self.country_id:
            self.state_id = False

    @api.onchange('state_id')
    def _onchange_state_id(self):
        """Sync country_id to match the country of the newly selected state."""
        if self.state_id.country_id and self.country_id != self.state_id.country_id:
            self.country_id = self.state_id.country_id

    @api.onchange('region_id')
    def _onchange_region_id(self):
        """Reset ICB/ICS/Health Board fields and re-scope their domains to the newly selected region."""
        self.icb_id = False
        self.ics_id = False
        self.health_board_id = False
        if self.region_id:
    ...
    def _onchange_icb_id(self):
        """Reset ics_id, derive region_id from the selected ICB, and re-scope the ics_id/region_id domains."""
        self.ics_id = False
    ...
    def _onchange_health_board_id(self):
        """Derive region_id from the selected Health Board and re-scope the region_id domain."""
        domain = {}
    ...
    def _compute_board_member_count(self):
        """Count the linked board members for the stat button."""
        for trust in self:
            trust.board_member_count = len(trust.board_member_ids)

    @api.model_create_multi
    def create(self, vals_list):
        """Extend create() to uppercase ods_code before delegating to super()."""
    ...
    def write(self, vals):
        """Extend write() to enforce the workflow state machine and uppercase ods_code.

        Direct writes to `state` are blocked unless the target state is a permitted
        transition from the current state (see allowed_transitions) and the
        'approved_state_change' context flag is set, otherwise raises UserError.
        Callers should go through the State Change Wizard instead of writing
        `state` directly. Also raises ValidationError if the Trust is already
        'dissolved' (a terminal state with no further transitions).
        """
        if 'state' in vals:
    ...
    def copy_data(self, default=None):
        """Extend copy_data() to append '(copy)' to the name and generate a fresh unique ods_code.

        The new ods_code is derived from the original by mutating trailing
        characters until an unused code is found (falls back to a fully random
        code after 100 failed attempts), since ods_code has a DB uniqueness
        constraint and duplicating it verbatim would fail on save.
        """
    ...
    def action_open_state_change_wizard(self):
        """Open the State Change Wizard as a modal, pre-filled with this Trust."""
        self.ensure_one()
    ...
    def unlink(self):
        """Extend unlink() to only allow deleting Trusts still in 'draft' state and with no linked sites."""
        for trust in self:
