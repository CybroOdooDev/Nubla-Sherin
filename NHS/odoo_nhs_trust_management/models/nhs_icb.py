...
class NhsIcb(models.Model):
    ...
    def _check_region_system(self):
        """Enforce that an ICB only links to an NHS England region; raises ValidationError otherwise."""
        for icb in self:
    ...
    def _compute_trust_count(self):
        """Compute the number of trusts linked to the ICB via trust_ids."""
        for icb in self:
    ...
    def copy_data(self, default=None):
        """Append '(copy)' to the name and generate a unique ODS code when duplicating an ICB."""
