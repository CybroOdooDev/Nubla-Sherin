...
class NhsHealthBoard(models.Model):
    ...
    def _check_region_system(self):
        """Enforce that an NHS Scotland Health Board only links to an NHS Scotland region; raises ValidationError otherwise."""
        for board in self:
    ...
    def _compute_trust_count(self):
        """Compute the number of trusts linked to the board via trust_ids."""
        for board in self:
            board.trust_count = len(board.trust_ids)

    def copy_data(self, default=None):
        """Append '(copy)' to the name and generate a unique ODS code when duplicating a health board."""
