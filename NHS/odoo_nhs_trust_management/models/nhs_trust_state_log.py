...
class NhsTrustStateLog(models.Model):
    ...
    def _compute_display_name(self):
        """Build a human-readable "<Trust> → <Target State>" label for each log entry."""
        for log in self:
    ...
    def write(self, vals):
        """Block all edits, since these audit log entries must remain immutable."""
        raise UserError('Workflow change logs are immutable audit records and '
                        'cannot be modified under any circumstances!')

    def unlink(self):
        """Restrict deletion of audit log entries to System Administrators, then delegate to super()."""
