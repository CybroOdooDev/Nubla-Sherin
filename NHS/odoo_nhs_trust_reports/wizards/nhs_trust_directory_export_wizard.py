...
class NhsTrustDirectoryExport(models.TransientModel):
    """Wizard to export the NHS Trust directory (filtered by health system,
    status and region) to a styled Excel workbook."""
    _name = 'nhs.trust.directory.export.wizard'
    ...
    def _onchange_health_system(self):
        """Clear the selected regions when the health system changes, since
        region_ids is scoped to whichever health system is active."""
        self.region_ids = False

    # ── Domain helper

    def _build_domain(self):
        """Build the nhs.trust search domain from the health system, status
        and region filters selected on the wizard."""
        domain = []
    ...
    def action_export(self):
        """Generate the Excel directory export and attach it to the wizard
        record for download."""
        self.ensure_one()
        try:
