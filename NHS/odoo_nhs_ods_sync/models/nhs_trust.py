...
class NhsTrust(models.Model):
    ...
    def _compute_ods_pending_conflict_count(self):
        ...
        Uses sudo() because this is just a count, shown to users (e.g. via the
        conflict ribbon/smart button) who are not themselves granted access to
        nhs.ods.sync.conflict — without it, merely opening a trust form as a
        non-manager/operator user raises an AccessError."""
        for trust in self:
            trust.ods_pending_conflict_count = self.env['nhs.ods.sync.conflict'].sudo().search_count([
                ('trust_id', '=', trust.id),
                ('state', '=', 'pending'),
            ])
    ...
    def _check_geographic_fields(self):
        ...
        if self.env.context.get('nhs_ods_sync'):
            return
        if hasattr(super(), '_check_geographic_fields'):
            super()._check_geographic_fields()
    ...
    def _check_governance_link(self):
        ...
        if self.env.context.get('nhs_ods_sync'):
            return
        if hasattr(super(), '_check_governance_link'):
            super()._check_governance_link()
