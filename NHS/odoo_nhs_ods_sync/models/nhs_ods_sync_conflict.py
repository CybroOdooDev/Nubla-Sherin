...
class NhsOdsSyncConflict(models.Model):
    ...
    def action_use_ods(self):
        ...
        For Many2one fields (e.g. trust_type_id) the conflict stores only the
        human-readable name in ods_value (a Char field). Before writing we
        resolve that name to the related record's database ID so that the write
        does not raise a "mandatory field not set" validation error.
        """
        for rec in self.filtered(lambda r: r.state == 'pending'):
            if rec.trust_id and rec.field_name and rec.ods_value is not None:
                ctx = {'nhs_ods_sync': True, 'approved_state_change': True}
                write_value = rec.ods_value or False
                # Resolve Many2one names to their record IDs before writing
                trust_fields = self.env['nhs.trust']._fields
                field_def = trust_fields.get(rec.field_name)
                if field_def and field_def.type == 'many2one' and rec.ods_value:
                    related_rec = self.env[field_def.comodel_name].search(
                        [('name', '=', rec.ods_value)], limit=1
                    )
                    if not related_rec:
                        raise UserError(
                            _("Cannot resolve ODS value '%(val)s' to a "
                              "'%(model)s' record. "
                              "Please update the local value manually.") % {
                                'val': rec.ods_value,
                                'model': field_def.comodel_name,
                            }
                        )
                    write_value = related_rec.id
                rec.trust_id.with_context(**ctx).write({rec.field_name: write_value})
            rec.write({
