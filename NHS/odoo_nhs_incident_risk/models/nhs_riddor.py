...
class NhsRiddor(models.Model):
    """A RIDDOR reportability determination and HSE report log for an incident."""
    _name = 'nhs.riddor'
    ...
    def _compute_display_name(self):
        """Build a display name from the linked incident, or the record id if unlinked."""
        for rec in self:
            if rec.incident_id:
    ...
    def _compute_deadline(self):
        """Compute the statutory HSE report deadline from the incident date and RIDDOR category."""
        for rec in self:
