...
class NhsInvestigation(models.Model):
    """Investigation or learning response opened against an incident, PSIRF-aligned."""
    ...
    def create(self, vals_list):
        """Assign a sequence-based reference to new investigations."""
    ...
    def _check_psii_tor(self):
        """Require Terms of Reference for PSII investigations once they leave draft."""
        for rec in self:
    ...
    def action_start(self):
        """Move the investigation to the In Progress state."""
        self.write({'state': 'in_progress'})

    def action_submit(self):
        """Validate required fields and submit the investigation for approval."""
        for rec in self:
    ...
    def action_approve(self):
        """Approve the investigation once all linked actions are closed.

        Restricted to Quality Lead users. Also advances the parent incident
        to the Actions stage if it was still in the Investigation stage.
        """
    ...
    def action_rework(self):
        """Move the investigation back to the In Progress state for rework."""
        self.write({'state': 'in_progress'})

    def action_print_report(self):
        """Generate and return the investigation summary report action."""
        self.ensure_one()
        return self.env.ref(
