...
class ResCompany(models.Model):
    """Extend res.company with NHS provider-type and incident-reporting configuration."""
    ...
    def _get_public_form_token(self):
        """Return the public form security token, generating one if it doesn't exist yet."""
        self.ensure_one()
...
class ResConfigSettings(models.TransientModel):
    """Expose NHS incident and reporting configuration fields in the Settings UI."""
    ...
    def action_regenerate_token(self):
        """Regenerate the public incident report form's security token and reload the view."""
        self.company_id.sudo().write(
            {'public_form_token': str(uuid.uuid4()).replace('-', '')[:20]})
        return {'type': 'ir.actions.client', 'tag': 'reload'}

    def action_open_public_form(self):
        """Open the public incident report form URL in a new browser tab."""
        self.ensure_one()
