...
class NhsCqcNotificationType(models.Model):
    """CQC statutory notification type reference data."""
...
class NhsCqcNotification(models.Model):
    """A CQC statutory notification raised against an incident."""
    ...
    def action_submit(self):
        """Mark the notification as submitted, stamping the submission time and user if unset."""
        for rec in self:
    ...
    def action_not_required(self):
        """Mark the notification as not required."""
        self.write({'state': 'not_required'})

    def action_required(self):
        """Reset the notification back to required, clearing submission details."""
        self.write({
            'state': 'required',
            'submitted_at': False,
    ...
    def _compute_display_name(self):
        """Build a display name combining the incident name and notification type."""
        for rec in self:
