...
class NhsIncidentCategory(models.Model):
    """A two-level incident category tree used to classify incidents."""
    ...
    def _compute_complete_name(self):
        """Build the full category path from the parent category's complete name."""
        for rec in self:
            if rec.parent_id:
    ...
    def _check_depth(self):
        """Ensure categories are nested no more than 2 levels deep."""
        for rec in self:
    ...
    def _compute_display_name(self):
        """Use the complete name as the display name, falling back to the plain name."""
        for rec in self:
