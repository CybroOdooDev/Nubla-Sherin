...
class NhsLocation(models.Model):
    """A physical location in a site → unit/ward → room hierarchy."""
    _name = 'nhs.location'
    ...
    def _compute_complete_name(self):
        """Build the full location path from the parent location's complete name."""
        for rec in self:
            if rec.parent_id:
    ...
    def _check_depth(self):
        """Ensure locations are nested no more than 3 levels deep."""
        for rec in self:
