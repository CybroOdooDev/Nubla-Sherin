...
class NhsIncident(models.Model):
    ...
    def create(self, vals_list):
        """Assign a sequence-based reference and apply harm/notification rules and the default handler."""
        ...
        for rec in records:
            ...
            if not rec.handler_id and rec.location_id.default_handler_id:
                # sudo: assigning the default handler is system automation, and the
                # creating user (e.g. a Reporter) has no write access to nhs.incident.
                rec.sudo().with_context(nhs_workflow=True).write(
                    {'handler_id': rec.location_id.default_handler_id.id})
        return records

    def write(self, vals):
        """Block direct state changes outside workflow actions, and re-apply harm/notification rules when harm grade or category change."""
    ...
    def _apply_harm_rules(self):
        """Force PSII response for Never Events and auto-create a Duty of Candour record once the harm grade meets the trigger threshold.

        Runs as sudo: this is system automation triggered on incident create/write,
        and the triggering user (e.g. a Reporter) has no write access to the incident's
        compliance fields and no create access on nhs.duty.of.candour (see evaluate()
        on nhs.notification.rule for the same pattern).
        """
        for rec in self:
            rec = rec.sudo()
            ...
            if rec.harm_grade and rec.harm_grade in DOC_TRIGGER_GRADES:
                ...
                if grade_idx >= trigger_idx and not rec.doc_id:
                    doc = self.env['nhs.duty.of.candour'].sudo().create({
                        'incident_id': rec.id,
    ...
    def action_accept(self):
        """Move the incident to the Triage state."""
        for rec in self:
            rec.with_context(nhs_workflow=True).write({'state': 'triage'})

    def action_reject(self, reason):
        """Reject the incident with a reason and email feedback to the reporter, if not anonymous."""
        for rec in self:
    ...
    def action_mark_duplicate(self, master_id):
        """Mark the incident as a duplicate of the given master incident."""
        for rec in self:
            rec.with_context(nhs_workflow=True).write({
                'state': 'duplicate',
    ...
    def action_advance_to_actions(self):
        """Advance the incident to the Actions state once its investigation is approved."""
        for rec in self:
    ...
    def action_request_closure(self):
        """Request closure, ensuring all actions, the Duty of Candour record, and CQC notifications are resolved first."""
        for rec in self:
    ...
    def action_close(self):
        """Close the incident and send closure feedback to the reporter, if not anonymous."""
    ...
    def action_create_risk(self):
        """Create a new risk register entry from this incident."""
        self.ensure_one()
        return self.env['nhs.risk'].create_from_incident(self)

    def action_open_triage_wizard(self):
        """Open the triage wizard for this incident."""
        self.ensure_one()
        return {
    ...
    def action_open_riddor_wizard(self):
        """Open the RIDDOR determination wizard for this incident."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_open_persons(self):
        """Open the list of persons affected by this incident."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_open_actions(self):
        """Open the list of actions raised against this incident."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Actions',
    ...
    def action_open_investigation(self):
        """Open the linked investigation record."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Investigation',
