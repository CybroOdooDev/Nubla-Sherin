...
class NhsDutyOfCandour(models.Model):
    """Duty of Candour compliance record tracking CQC Regulation 20 notification stages for an incident."""
    ...
    def _compute_display_name(self):
        """Compute a human-readable display name combining the linked incident's reference."""
        for rec in self:
            if rec.incident_id:
    ...
    def _compute_written_deadline(self):
        """Compute the written notification deadline as 10 working days after the trigger date."""
    ...
    def _compute_state(self):
        """Compute the compliance state from stage completion, the deadline, and any recorded exemption."""
    ...
    def action_generate_letter(self):
        """Generate and return the Duty of Candour letter report action."""
        self.ensure_one()
        return self.env.ref(
    ...
    def _cron_doc_deadlines(self):
        """Notify Quality Lead users when a written notification deadline is 5 or 2 days away."""
        today = fields.Date.today()
        ...
        quality_users = quality_group.all_user_ids if quality_group else self.env['res.users']
        template = self.env.ref(
            'odoo_nhs_incident_risk.mail_template_doc_reminder', raise_if_not_found=False)
        for doc in docs:
            ...
            if days_left in (2, 5):
                for user in quality_users:
                    ...
                    if template and user.email:
                        template.send_mail(
                            doc.id, email_values={'email_to': user.email}, force_send=True)
