...
class NhsProviderSetupWizard(models.TransientModel):
    """First-run wizard to activate a provider-type preset for the company."""
    ...
    def _compute_preview(self):
        """List the top-level incident categories applicable to the selected provider type."""
        for rec in self:
    ...
    def action_apply(self):
        """Apply the chosen provider type: set it on the company, ensure a public
        form token exists, and archive incident categories not applicable to it."""
        self.ensure_one()
