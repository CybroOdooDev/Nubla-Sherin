...
class NhsTriageWizard(models.TransientModel):
    """Wizard to triage a reported incident: confirm category/location/harm,
    choose a PSIRF response level, and accept, reject, or mark it as duplicate."""
    ...
    def _onchange_incident(self):
        """Pre-fill category, location, and response level from the selected incident."""
        if self.incident_id:
    ...
    def _onchange_category(self):
        """Suggest the response level configured as the default for the chosen category."""
    ...
    def action_confirm(self):
        """Apply the triage decision: reject or mark as duplicate, or otherwise
        write the confirmed triage data to the incident, move it into the
        'triage' state, and re-evaluate notification rules."""
        self.ensure_one()
