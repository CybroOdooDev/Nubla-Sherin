...
class NhsComplaintSubject(models.Model):
    """A KO41a-aligned subject or sub-subject category used to classify complaints for the annual statutory return."""
    ...
    def _compute_complete_name(self):
        """Build the full hierarchical name by prefixing the parent subject's complete name."""
        for rec in self:
            if rec.parent_id:
    ...
    def _check_two_levels(self):
        """Reject a subject whose parent already has a parent, enforcing a maximum of two hierarchy levels."""
        for rec in self:
    ...
    def _compute_display_name(self):
        """Use the complete hierarchical name (falling back to the plain name) as the record's display name."""
        for rec in self:
