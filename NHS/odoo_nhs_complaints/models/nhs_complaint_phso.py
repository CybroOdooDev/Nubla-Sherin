...
class NhsComplaintPhso(models.Model):
    """Tracks a complaint's escalation to, and decision from, the Parliamentary and Health Service Ombudsman."""
    ...
    def _check_outcome_details_allowed(self):
        """Prevent setting outcome, compensation or recommendations before a PHSO decision has been made."""
        for rec in self:
    ...
    def unlink(self):
        """Prevent deletion of PHSO escalation records since they are statutory records."""
        raise UserError(
            'PHSO escalation records are statutory and cannot be deleted.'
        )

    def action_mark_under_review(self):
        """Move the PHSO escalation from Referred to Under Review."""
        self.write({'state': 'under_review'})

    def action_record_decision(self):
        """Move the PHSO escalation to the Decision Made state."""
        self.write({'state': 'decision_made'})

    def action_close(self):
        """Move the PHSO escalation to the Closed state."""
        self.write({'state': 'closed'})

    @api.model
    def _cron_phso_followup(self):
        """Schedule a weekly follow-up activity for every PHSO escalation still referred or under review."""
