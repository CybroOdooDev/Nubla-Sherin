...
class NhsComplainant(models.Model):
    """Represents the person (or representative) raising a PALS concern or formal complaint,
    including vexatious-complainant handling."""
    ...
    def _compute_complaint_count(self):
        """Set the count of complaints linked to this complainant."""
        for rec in self:
            rec.complaint_count = len(rec.complaint_ids)

    @api.constrains('is_vexatious', 'vexatious_note')
    def _check_vexatious_note(self):
        """Require a handling policy note whenever a complainant is flagged as vexatious."""
        for rec in self:
    ...
    def action_view_complaints(self):
        """Open the list/form view of complaints filed by this complainant."""
        return {
