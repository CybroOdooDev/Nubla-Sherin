...
class PublicComplaintController(http.Controller):
    """Serves the public-facing complaint submission form and handles anonymous complaint intake via a signed token."""
    ...
    def public_complaint_form(self, token, **kw):
        """Render the public complaint form for the organisation identified by the given token."""
    ...
    def public_complaint_submit(self, token, **post):
        """Validate and create a PALS/complaint record from an anonymous public form submission."""
        company = self._get_company_by_token(token)
        if not company:
            return request.not_found()
    ...
    def _get_company_by_token(self, token):
        """Return the current company if the public form is enabled and the token matches, else None."""
