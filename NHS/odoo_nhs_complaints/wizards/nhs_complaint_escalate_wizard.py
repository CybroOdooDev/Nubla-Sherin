...
class NhsComplaintEscalateWizard(models.TransientModel):
    """Wizard to escalate a PALS concern into a new formal complaint record."""
    ...
    def _onchange_pals_id(self):
        """Pre-fill the escalation form fields from the selected PALS concern."""
        if self.pals_id:
    ...
    def action_escalate(self):
        """Create a new formal complaint from the PALS concern's details, mark the concern as Escalated, and open the new complaint."""
        self.ensure_one()
        pals = self.pals_id
