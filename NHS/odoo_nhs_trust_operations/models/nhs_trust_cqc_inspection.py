...
class NhsTrustCqcInspection(models.Model):
    """A Care Quality Commission inspection record for an England NHS Trust."""
    ...
    def _compute_display_name(self):
        """Build display_name as '<Trust short_name> — <inspection_date> — <rating>'."""
    ...
    def action_submit(self):
        """Move the inspection from Draft to Under Review."""
        for rec in self:
            rec.state = 'under_review'

    def action_approve(self):
        """Move the inspection from Under Review to Active."""
        for rec in self:
            rec.state = 'active'

    def action_reset_to_draft(self):
        """Reset the inspection back to Draft."""
        for rec in self:
            rec.state = 'draft'

    @api.constrains('inspection_date', 'next_inspection_due')
    def _check_dates(self):
        """Raise if next_inspection_due is not strictly after inspection_date."""
        for rec in self:
    ...
    def _check_trust_id(self):
        """Raise if the inspection's trust is not part of the England health system."""
        for rec in self:
