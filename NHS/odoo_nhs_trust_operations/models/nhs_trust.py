...
class NhsTrustOperations(models.Model):
    ...
    def _compute_site_count(self):
        """Set site_count to the number of sites linked to the trust, for the stat button."""
        for trust in self:
            trust.site_count = len(trust.site_ids)

    @api.depends('site_ids.department_ids')
    def _compute_ops_department_count(self):
        """Set department_count to the total departments across all of the trust's sites."""
        for trust in self:
    ...
    def _compute_total_bed_capacity(self):
        """Set total_bed_capacity from manual_bed_capacity if set, else sum site bed_capacity."""
        for trust in self:
    ...
    def _compute_surplus_deficit(self):
        """Set surplus_deficit to annual_income minus annual_expenditure."""
        for trust in self:
    ...
    def _compute_latest_cqc(self):
        """Derive CQC registration/status/rating/date from the most recent inspection.

        Scotland trusts are not CQC-regulated, so their CQC fields are cleared and
        cqc_registration_status is forced to 'not_applicable'. For England trusts,
        the registration status and workflow state come from the most recent
        inspection by inspection_date, while the rating and date are only taken
        from the most recent inspection that is in the 'active' state.
        """
        for trust in self:
    ...
    def action_view_sites(self):
        """Open the list/form view of sites belonging to this trust (stat button)."""
        self.ensure_one()
        return {
            'name': 'Sites',
    ...
    def action_view_departments(self):
        """Open the list/form view of departments belonging to this trust (stat button)."""
        self.ensure_one()
        return {
            'name': 'Departments',
            'type': 'ir.actions.act_window',
    ...
    def action_view_cqc_inspections(self):
        """Open the list/form view of CQC inspections belonging to this trust (stat button)."""
        self.ensure_one()
        return {
