...
class NhsTrustDepartment(models.Model):
    """A sub-unit of an NHS Trust site (clinical, corporate, support, or research)."""
    ...
    def copy_data(self, default=None):
        """Suffix the duplicated department's name with '(copy)' unless overridden in default."""
    ...
    def create(self, vals_list):
        """Block creating a department under a site whose parent trust is dissolved."""
    ...
    def write(self, vals):
        """Block relinking to, or modifying a department under, a dissolved trust's site."""
        if 'site_id' in vals and vals['site_id']:
            site = self.env['nhs.trust.site'].browse(vals['site_id'])
            if site.trust_id.state == 'dissolved':
