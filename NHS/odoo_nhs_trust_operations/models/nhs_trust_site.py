...
class NhsTrustSpecialty(models.Model):
    """Lookup table of NHS Data Dictionary clinical specialties (e.g. Cardiology)."""
    ...
    def _check_name_code_unique(self):
        """Raise if another specialty record already uses the same name or code.

        Duplicates the DB-level UNIQUE constraints (_name_unique, _code_unique) to
        surface a friendlier, field-specific error message in the UI.
        """
        for rec in self:
    ...
    def copy_data(self, default=None):
        """Suffix the duplicated specialty's name with '(copy)' unless overridden in default."""
...
class NhsTrustSite(models.Model):
    """A physical location belonging to an NHS Trust (hospital, clinic, ambulance station, etc.)."""
    ...
    def _compute_department_count(self):
        """Set department_count to the number of departments based at this site, for the stat button."""
        for site in self:
            site.department_count = len(site.department_ids)

    def action_view_departments(self):
        """Open the list/form view of departments based at this site (stat button)."""
        self.ensure_one()
        return {
            'name': 'Departments',
    ...
    def copy_data(self, default=None):
        """Suffix the duplicated site's name with '(copy)' unless overridden in default."""
        default = dict(default or {})
        vals_list = super().copy_data(default=default)
        if 'name' not in default:
    ...
    def create(self, vals_list):
        """Block creating a site under a dissolved trust."""
    ...
    def write(self, vals):
        """Block relinking to, or modifying a site belonging to, a dissolved trust."""
        if 'trust_id' in vals and vals['trust_id']:
            trust = self.env['nhs.trust'].browse(vals['trust_id'])
            if trust.state == 'dissolved':
    ...
    def unlink(self):
        """Block deleting a site that still has departments; they must be removed or relocated first."""
        for site in self:
