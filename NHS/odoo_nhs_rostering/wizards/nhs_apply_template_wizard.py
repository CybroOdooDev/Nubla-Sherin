...
class NhsApplyTemplateWizard(models.TransientModel):
    ...
    rotation_template_id = fields.Many2one(
        ...
        domain="[('roster_unit_id', '=', unit_id)]", help="Rotation Template")
    unit_id = fields.Many2one(related='period_id.unit_id', string='Unit', help="Unit")
    member_ids = fields.Many2many(
        'nhs.workforce.member', string='Apply To', required=True,
        domain="[('id', 'in', team_member_ids)]", help="Apply To")
    ...
    def action_apply(self):
        ...
        while a_date <= self.period_id.date_end:
            ...
            a_date += timedelta(days=1)
        message = ('%d duties assigned.') % created
        if skipped:
            message += (' %d skipped (rule violations) - see below:\n%s') % (
                len(skipped), '\n'.join(skipped[:20]))
        return {
