...
class NhsOdsSyncConflict(models.Model):
    ...
    current_value = fields.Char(string='Current Value (local)')
    ods_value = fields.Char(string='ODS Value')
        ...
        for rec in self.filtered(lambda r: r.state == 'pending'):
            ...
            if rec.trust_id:
                rec.trust_id.message_post(
                    body=_(
                        "ODS Conflict on '%(field)s' resolved: kept local value '%(val)s'.",
    ...
    def action_use_ods(self):
        for rec in self.filtered(lambda r: r.state == 'pending'):
            ...
            if rec.trust_id:
                rec.trust_id.message_post(
                    body=_(
                        "ODS Conflict on '%(field)s' resolved: accepted ODS value '%(val)s'.",
                        field=rec.field_label or rec.field_name,
    ...
    def action_ignore(self):
        for rec in self.filtered(lambda r: r.state == 'pending'):
            if not rec.resolution_note:
                raise UserError(_("Please provide a resolution note before ignoring a conflict."))
            ...
            if rec.trust_id:
                rec.trust_id.message_post(
                    body=_(
                        "ODS Conflict on '%(field)s' ignored. Reason: %(note)s",
                        field=rec.field_label or rec.field_name,
