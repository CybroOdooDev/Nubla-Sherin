...
class PosReceiptLayoutClientAction extends Component {
        ...
        this.editor = new MediumEditor(this.receiptContentRef.el, {
            toolbar: {
                buttons: ['bold', 'italic', 'underline', 'strikethrough',
                 'subscript', 'superscript', 'h1', 'h3', 'quote', 'anchor'],
            },
            placeholder: false,
            targetBlank: true,
    ...
    async loadReceipt(reset = false) {
        ...
        if (typeof html !== 'string') {
            if (html === null || html === undefined || html === false) {
                ...
                this.notification.add(`Invalid receipt design format (${typeof html}). Expected XML string.`,
                 { type: 'warning' });
                html = String(html);
            }
        }
    ...
    async restoreSavedColumnsDesign3() {
        try {
            ...
            let retries = 0;
            const maxRetries = 10;
            while ((!this.receiptContentRef?.el || !this.receiptContentRef.el.querySelector(".receipt-header-dropzone"))
             && retries < maxRetries) {
                await new Promise(resolve => setTimeout(resolve, 200));
                retries++;
            }
